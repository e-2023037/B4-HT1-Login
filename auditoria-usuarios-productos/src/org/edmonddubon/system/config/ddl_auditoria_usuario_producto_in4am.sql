...
create table Users(
	name varchar(50) not null check(length(nombres)<=50),
    lastname varchar(50) not null check(length(apellidos)<=50),
    email varchar(50) not null check(length(email)<=50),
    user varchar(25) not null check(length(password)<=35),
    password varchar(35) not null check(length(password)<=35),
    id_user varchar(36) not null,
    constraint pk_users primary key (id_user)
);

-- uso de constrain pa' validar
Delimiter $$
